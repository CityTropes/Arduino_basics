...
void loop() 
{
  ...
  if (digitalRead(buttonApin) == LOW)  //if button A is pressed and circuit is broken (input pin to GND)
  {
    digitalWrite(ledPin, HIGH);       //turn on connected LED
  }
  if (digitalRead(buttonBpin) == LOW) //if button B is pressed and circuit is broken (input pin to GND)
  {
